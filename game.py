...
class Player():
    ...
    def move(self,map_id):
        ...
        if pressed_keys[K_DOWN] and self.horizon > 0:
            self.horizon -=10
        #Takes keyboard input and determines the vector of the players position delta, using the player angle, and adding or subtracting PI/2 for left and right, and subtracting the delta for back
        if pressed_keys[K_d]:
        ...
        mousepos = pygame.mouse.get_pos() #get the location of the mouse on the window
        self.looking = (RES_W/2 - mousepos[0], RES_H/2 - mousepos[1]) #the displacement of the mouse from the center of the screen
        pygame.mouse.set_pos(RES_W/2, RES_H/2) #put the mouse back to the center of the screen
        self.angle += self.looking[0]/-1000 #add the displacement to the camera
        self.horizon += self.looking[1]
        
        if self.angle > 2 * PI :self.angle -= 2 * PI #Loop the angle back around if it makes a full circle, or goes into a negative circle
        if self.angle < 0 : self.angle += 2 * PI
        
        if self.horizon < 0    : self.horizon = 0    #Limit the horizon to half the total resolution (this is about a 90 degree range of view)
        if self.horizon >RES_H : self.horizon = RES_H
        
        self.delt *= 5
        newmx, newmy = int((self.pos.x+self.delt.x)/blockRes),int((self.pos.y+self.delt.y)/blockRes) #checks if the player will be in a wall
        if map[newmy*mapx+newmx]==0:                                                         #after moving and prevents it if so
            self.pos +=self.delt
    
P1 = Player() #make the player object
...
def drawRay(r,posx, posy,height,offset,distance,orientation, texture = pygame.Surface):
    ...
    #draw floor
#
def castRay(rayNum,rayAng,map_id) :             
    ...
    if rayAng > 2*PI: rayAng -=2*PI
    if rayAng < 0   : rayAng +=2*PI
    dof=0
    hx,vx = P1.pos.x,P1.pos.x
    hy,vy = P1.pos.y,P1.pos.y
    dish,disv = float(1000000),float(1000000)
    nTan= -(math.tan(rayAng));aTan=-1/math.tan(rayAng)
    #check horizontal lines
    if rayAng > PI : #facing upwards
        ...
        yo =-blockRes
        xo = -yo * aTan
    if rayAng < PI : #facing downwards
        ry = (int(P1.pos.y / blockRes)) * blockRes + blockRes;rx = (P1.pos.y - ry) * aTan + P1.pos.x;yo = blockRes;xo = -yo * aTan
    if rayAng == 0 or rayAng == PI : #facing parrellel to x axis
        rx = P1.pos.x;ry = P1.pos.y;dof = maxdof
    while dof < maxdof :
        mx = int(rx / blockRes) #mx and my are the grid spaces the ray is in
        my = int(ry / blockRes)
        mp = my * mapx + mx #index value of the coordinates (mx,my)
        if mp > 0 and mp < mapx * mapy and map[mp]>0 : #check index of current ray position
            dof = maxdof
            hx = rx
            hy = ry     #if hit save ray data
            dish = math.dist((hx, hy), (P1.pos.x, P1.pos.y))
            htex = map[mp]
        else :
            rx += xo
            ry += yo #if no hit, iterate again
            dof += 1
    #check vertical lines
    dof=0
    if rayAng > P2 and rayAng < P3 :
        rx = (int(P1.pos.x / blockRes)) * blockRes - 0.0001
        ry = (P1.pos.x - rx) * nTan + P1.pos.y
        xo =-blockRes
        yo = -xo * nTan
    if rayAng < P2 or rayAng > P3 :
        rx = (int(P1.pos.x / blockRes)) * blockRes + blockRes
        ry = (P1.pos.x - rx) * nTan + P1.pos.y
        xo = blockRes
        yo = -xo * nTan
    if rayAng == 0 or rayAng == PI :
        ry = P1.pos.y
        rx = P1.pos.x
        dof = maxdof
    while dof < maxdof :
    ...
    if disv>dish :
        ...
        tex = htex
    disT *=  math.cos((P1.angle-rayAng)) #fisheye correction
    lineH = (blockRes * RES_H)/disT #calculate the height of the wall depending on distance
    if lineH > 2500: lineH = 2500 #limit the line size to fix problem with pygame.transform.scale() absolutely tanking performance.
    lineO = P1.horizon - lineH/2 #calculate how far from the horizon the line needs to be drawn
    drawRay(rayNum,rx,ry,lineH,lineO,disT,orientation,textures[tex])
...
def displayText():
    frameRate = str(int(gameClock.get_fps())) #gets the fps from the clock object called gameClock, convert it to an INT to round it, then to a string to get printed
    pygame.Surface.blit(win,gameFont.render("FPS: "+frameRate,False,(255,0,0)),(10,10)) #print the fps to the screen
    controlsText = "Controls: WASD - Move | Arrow Keys/Mouse - Camera | T - Enable Night Mode (VERY Laggy) | O - Open Map Editor | L - Load Map"
    pygame.Surface.blit(win,gameFont.render(controlsText,False,(255,0,0)),(10,50))
    
    

map = readMapData("1.txt")

def runLevel():
    global isDaytime,map
    ...
    while running:
        ...
        if isDaytime:
            win.fill('sky blue');pygame.draw.rect(win,(100,100,100),(0,P1.horizon,RES_W,RES_H))
        else:
            win.fill('black');pygame.draw.rect(win,(25,25,25),(0,P1.horizon,RES_W,RES_H))#draws sky and floor depending on if its daytime or not
        
        
        P1.move(0)
        RayCasting() #move the player and raycast
        
        
        cursor = pygame.Rect(RES_W/2,RES_H/2,4,4) 
        pygame.draw.rect(win,(255,0,0),cursor)    #draw the cursor in the center of the screen
